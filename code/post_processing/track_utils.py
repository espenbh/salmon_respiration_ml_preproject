...
# Dists: A timesequence of euclidean distances between the lower and upper jaw of the fish.
...
def update_trackers(active_tracker, log_tracker, frame, targets, box_count, NN_iou_threshold = 0.1, track_iou_treshold = 0.1):
    ...
    for idx in range(len(targets)):
        ...
        # Chack all fishes registered in the current frame
        for i in nms:
            hit = 0
            ...
            for ab in range(len(active_tracker)):
                if bb_intersection_over_union(active_tracker[ab]['last_bbox'], boxes[i]) > track_iou_treshold:
                    active_tracker[ab]['last_bbox'] = boxes[i]
                    active_tracker[ab]['last_keypoints'] = keypoints[i]
                    active_tracker[ab]['dists'].append(math.dist(targets[idx]['keypoints'][i][0], targets[idx]['keypoints'][i][1]))
            ...
            if hit == 0:
                new_inst = {'last_bbox': boxes[i], 
                    'dists': [math.dist(targets[idx]['keypoints'][i][0], targets[idx]['keypoints'][i][1])], 
                    'first_frame': frame, 
                    'last_frame': frame, 
                    'last_keypoints': keypoints[i],
                    'box_count': box_count}
